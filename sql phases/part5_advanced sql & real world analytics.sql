...
where total_course_value>(select avg(enrolls) from student_value);
...
select c.course_id,c.course_name,count(e.enrollment_id)*sum(c.price) as revenue
...
select distinct s.student_id,s.full_name,cp.completed_percent
...
having cp.completed_percent<20;
...
select c.course_id,c.course_name,avg(cp.completed_percent) as avg_completion
from courses c join enrollments e on c.course_id=e.course_id
join course_progress cp on e.enrollment_id=cp.enrollment_id
group by c.course_id,c.course_name
having avg_completion>55;



-- 8.categorize students into high,moderate,low engagement based on avg completion--

select c.course_id,c.course_name,avg(cp.completed_percent) as avg_completion,case 
...
PARTITION BY s.student_id ORDER BY cp.enrollment_id) AS prev_completion
FROM students s JOIN enrollments e ON s.student_id = e.student_id
JOIN course_progress cp ON e.enrollment_id = cp.enrollment_id
) t
